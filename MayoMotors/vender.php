...
if (empty($matricula)) {
    ...
    if (empty($errores)) {
        try {
            ...
            $column_check = $conn->query("SHOW COLUMNS FROM Coches LIKE 'tipo'");
            $tipo_exists = $column_check && $column_check->num_rows > 0;
            
            // Preparar consulta según la existencia de la columna tipo
            if ($tipo_exists) {
